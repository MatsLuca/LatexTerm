...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// Extra pixels added to each cell's height on top of the font's natural line height.
    /// Use this to introduce visible spacing between text rows. Recomputes layout when changed.
    public var extraLineSpacing: CGFloat = 0 {
        didSet {
            guard oldValue != extraLineSpacing else { return }
            resetFont()
        }
    }

    /// The pixel size of a single terminal cell with the current font and line spacing.
    public var lineCellSize: CGSize {

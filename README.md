...
The first pane on launch (and every `⌘N`) is a **home pane** instead of a shell: the folder tree of your projects on the left (Finder order; ▣ project, ▤ area, ● where a Claude pane is running), and on the right the actions for the selected folder — **↻ Weiter** (resume the last session, titled from Claude Code's own `ai-title` entries), **＋ Neue Session**, **› Nur Shell**, then the most recently active projects below that folder and earlier sessions. Typing filters the tree, `→`/`←` expand/collapse or move between tree and actions, `⏎` runs the first action, `⌘⇧N` creates a new project folder in the selected one and hands it to `/neues-projekt`. `⌘P` pins a session (`⇧⇥` shows the pin screen), `⌘E` renames one — the custom title overrides Claude Code's `ai-title` (`projekte rename <id> [title]`, empty = back to automatic). `⌘T` stays a plain shell inheriting the focused pane's CWD.

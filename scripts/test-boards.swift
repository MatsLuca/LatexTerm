...
struct BoardTests {
    static func main() {
        ...
        // KI-Namen (26.09.): jedes Turn-Ereignis (Prompt, fertige Antwort) und jede neue Kachel stößt an, leere Bretter nie.
        let t0 = Date(timeIntervalSince1970: 0)
        var naming = BoardNaming()
        check(!naming.isDue(turns: 0, panes: ["a"], hasSession: false), "fresh empty board: not due")
        check(naming.isDue(turns: 0, panes: ["a"], hasSession: true), "restored session: due once")
        naming.checked(turns: 0, panes: ["a"], now: t0, newName: nil)
        check(naming.name == nil, "keep without name: still start-up")
        check(!naming.isDue(turns: 0, panes: ["a"], hasSession: true), "kept, nothing new: not due")
        check(naming.isDue(turns: 1, panes: ["a"], hasSession: true), "new turn event: due")
        naming.checked(turns: 1, panes: ["a"], now: t0, newName: "Brett Namen")
        check(naming.name == "Brett Namen", "name taken")
        check(!naming.isDue(turns: 1, panes: ["a"], hasSession: true), "named, nothing new: not due")
        check(naming.isDue(turns: 2, panes: ["a"], hasSession: true), "named: every new turn event")
        check(naming.isDue(turns: 1, panes: ["a", "b"], hasSession: true), "new pane: always")
        check(!naming.isDue(turns: 1, panes: [], hasSession: true), "closed pane only: not due")
        naming.checked(turns: 2, panes: ["a"], now: t0, newName: "")
